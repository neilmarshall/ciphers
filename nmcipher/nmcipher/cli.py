...
class Parser():
    """
    >>> encrypt_func = lambda k, s, m: m[::-1]
    >>> decrypt_func = lambda k, s, m: m[::-1]
    >>> hack_func = lambda _, s, m: m[::-1]
    >>> parser = Parser(encrypt_func, decrypt_func, hack_func)

    >>> parser(["enc", "4", "ABCDEF", "ABC"])
    'CBA'

    >>> parser(["dec", "4", "ABCDEF", "CBA"])
    'ABC'

    >>> parser(["hack", "brute", "ABCDEF", "CBA"])
    'ABC'
    """
    ...
    def __init__(self, encrypt_func, decrypt_func, hack_func):
        ...
        encrypt_parser.add_argument("symbols", type=str, help="encryption symbols")
        ...
        decrypt_parser.add_argument("symbols", type=str, help="decryption symbols")
        ...
        hack_parser.add_argument("symbols", type=str, help="decryption symbols")
    ...
    def __call__(self, args=None):
        ...
        if parsed_args.subcommand == "hack":
            return parsed_args.func(parsed_args.method, parsed_args.symbols, parsed_args.message)
        return parsed_args.func(parsed_args.key, parsed_args.symbols, parsed_args.message)
